...
// ChefConfigListLoader.java --
//
// ChefConfigListLoader.java is part of ElectricCommander.
//
...
public class ChefConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final ChefConfigList   m_configList;
    private final CgiRequestProxy m_cgiRequestProxy;
    private String                m_editorName;

    //~ Constructors -----------------------------------------------------------

    public ChefConfigListLoader(
            ChefConfigList   configList,
            Component       component,
            ...
            ChainedCallback callback)
    {
        ...
        m_cgiRequestProxy = new CgiRequestProxy(getPluginName(), "chef.cgi");
    }
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading Chef configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading Chef configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName("/plugins/EC-Chef/project/forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("chefcfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for Chef plugin '" + m_configPlugin + "'";
