...
sub main {
    ...
    my $berksfile_data =
      ( $ec->getProperty("berksfile_data") )->findvalue('//value')
      ->string_value;
    ...
    my $filename = "Berksfile";
    my $filePath = "";

    #Variable that stores the command to be executed
    my $command = " yes | berks init ";
    ...
    print "Running procedure BerksInit\n";
    ...
    if ( $cookbook_path && $cookbook_path ne '' ) {
        ...
        $filePath = File::Spec->catfile( $cookbook_path, $filename );
    }
    ...
    #If Berksfile is present then write the data to the file
    if ( $berksfile_data && $berksfile_data ne '' ) {
        open my $fh, '>>', $filePath or die "can't open $filename: $!";
        print $fh $berksfile_data;
        close $fh;
    }
